#### ALL THESE FUNCTIONS/SNIPPETS ARE BEING PLACED INTO CUSTOM MODULES IN EITHER CANA.DATASETS OR CANA.CUSTOM_MODULES ###

#################### Required for loading the PI3K/Regan networks ############################# if not using custom code base installed via 'python setup.py develop'
...
############################################# REQUIRED TO PLOT FANCY SCHEMATA ##########################################
    ...
    for out, pis in zip([1, 0], [pi1s, pi0s]):
        for pi in pis:
            ...
            for input in pi:
                ...
                ax1.add_artist(
                    Text(x + cwidth / 2, y + cwidth / 10 * 4, text=text, color=textcolor, va='center', ha='center',
            ...
            x += sepcxspace
            r = Rectangle((x, y), width=cwidth, height=cwidth, facecolor='black' if (out == 1) else 'white',
                          edgecolor='black')
            ax1.add_artist(
                Text(x - (sepcxspace / 2) - (cxspace / 2), y + cwidth / 10 * 4, text=':', color='black', va='center',
                     ha='center', fontsize=14, weight='bold', family='serif'))
            ax1.add_artist(
                Text(x + (cwidth / 2), y + cwidth / 10 * 4, text=out, color='white' if (out == 1) else 'black',
    ...
    for out, tss in zip([1, 0], [ts1s, ts0s]):
        for ts, pss, sss in tss:
            ...
            r = Rectangle((x, y), width=cwidth, height=cwidth, facecolor='black' if (out == 1) else 'white',
                          edgecolor='#4c4c4c')
            ax2.add_artist(
                Text(x - (sepcxspace / 2) - (cxspace / 2), y + cwidth / 2, text=':', color='black', va='center',
                     ha='center', fontsize=14, weight='bold', family='serif'))
            ax2.add_artist(
                Text(x + (cwidth / 2), y + cwidth / 10 * 4, text=out, color='white' if (out == 1) else 'black',
                     va='center', ha='center', fontsize=14, family='serif'))
